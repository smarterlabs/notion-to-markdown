...
export function createLine(str) {
    return `${str}\n`;
}
export function createText(block) {
    counter.resetCount();
    // 改行
...
export function createHeading(block) {
    counter.resetCount();
    const title = block.value.properties.title.flat(Infinity);
    // h1
...
export function createList(block) {
    counter.resetCount();
    const title = block.value.properties.title.flat(Infinity);
    // list
    return createLine(`- ${title[0]}`);
}
export function createOrderList(block) {
    const text = block.value.properties.title.flat(Infinity)[0];
    counter.count();
    return createLine(`${counter.getCount()}. ${text}`);
}
export function createImage(block) {
    counter.resetCount();
